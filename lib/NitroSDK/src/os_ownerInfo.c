...
void GetDsFirmwareUserSettings(OSOwnerInfo *info) {
    ...
    MTi_CpuCopy16(src->ncd.owner.nickname.str, info->nickName, 20);
    MTi_CpuCopy16(src->ncd.owner.comment.str, info->comment, 52);
    info->nickName[10] = 0;
    info->comment[26] = 0;
}
